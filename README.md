...
| 点歌 | 按歌名返回 3-5 首候选并回复序号确认，也可按 ID/链接直接点歌 | `/点歌 [歌名/ID/链接]` |
...
- 按歌名搜索时，机器人会先返回候选列表（通常 3-5 首，最多 5 首），用户在 `MUSIC_SELECTION_TIMEOUT` 秒内回复 `1`、`2` 等序号后才会下载并发送；候选超时会提示“这次点歌候选已超时，请重新点歌”。
